        ...
        setTimeout(async () => {
          try {
            const ingredientsResponse = await fetch(
              `${apiUrl}/workflow/ingredients/${threadId}`
            );
    ...
    async (userInput: string) => {
      ...
      try {
        const response = await fetch(
          `${apiUrl}/workflow/resume/${state.threadId}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_input: userInput }),
  ...
  const selectOption = useCallback(
    async (optionId: string) => {
      ...
      setState((prev) => ({
        ...
        selectedOption:
          prev.projectOptions.find((o) => o.option_id === optionId) || null,
      }));

      try {
        // Call backend to proceed with selected option
        const response = await fetch(
          `${apiUrl}/workflow/select-option/${state.threadId}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const selectConcept = useCallback(
    async (conceptId: string) => {
      ...
      setState((prev) => ({
        ...
        selectedConcept:
          prev.concepts.find((c) => c.concept_id === conceptId) || null,
      }));

      try {
        // Use existing select-concept endpoint
        const conceptIndex = state.concepts.findIndex(
          (c) => c.concept_id === conceptId
        );
        const response = await fetch(
          `${apiUrl}/workflow/select-concept/${state.threadId}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
